...
- 線の太さは均一（1.5pt）
- 四隅では1本のコーナーストランドが、隣接する辺のストランド端点を時計回りに接続し、輪郭線が途切れないようにする
...
**コーナーストランドの描画手順** (`_drawCornerStrand`):

四隅の角丸マージン領域に1本のストランドを描き、隣接する辺の毛並みを滑らかに接続する。

1. 引数として、前の辺の最後のストランドの終点 `start` と次の辺の最初のストランドの始点 `end`（いずれもキャンバス座標）を受け取る
2. コーナーの種別ごとに、外側へ斜めに突き出す制御点（`(±diagonalPeak, ±diagonalPeak)` 相当）とフィルを閉じる内側座標 `cornerInner`（バブル内部の `(±cornerMargin, ±cornerMargin)` 相当）を決定
3. `start → control → end` の二次ベジェ曲線で弧を描く
4. 弧の内側を `cornerInner` で閉じた多角形を背景色で塗りつぶし
5. 弧の輪郭線を均一な太さ（1.5pt）で描画

コーナー間の接続関係（時計回り）:

| コーナー | 始点 (`start`) | 終点 (`end`) |
|---------|----------------|--------------|
...
これにより、辺のストランドとコーナーストランドが同一のキャンバス座標で接続され、輪郭線が四隅で途切れない。
