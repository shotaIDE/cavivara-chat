...
知識エントリーの内容は、**外部に公開されている情報**か、**公開しても差し支えのない結社の世界観・慣習**のみとする。カヴィヴァラさんの回答は誰でも引き出せるため、知識エントリーに書いた内容はすべて公開情報として扱われる。

| 情報源 | 扱い |
| --- | --- |
| 公式サイト、note、YouTube、teket、X などの公開情報 | そのまま知識エントリーにできる |
| 内部連絡（Discord / LINE のエクスポート） | **エントリーに載せられるのは、結社の世界観・慣習・公開済みの制作物に関する記述のみ**。下記の「載せないもの」に該当する内容は、要約であっても載せない |

内部連絡から載せないもの:

- 個人が特定できる情報（本名、連絡先、体調・家庭・勤務先の事情、退職や休職の理由など）
- 認証情報（アカウントの ID・パスワード）、および共有ドライブ・スプレッドシート・フォームなどの限定共有 URL
- 金銭に関する内部情報（費用の徴収、売上の処理、支払い先）
- 公演前の未公表情報（アンコール曲、次回公演の候補曲、進行表など）。公演のネタバレになる
- 未確定の予定や検討中の案

内部連絡のエクスポートそのものはリポジトリーにコミットしない（`.gitignore` で `/doc/reference/` を除外している）。
...
- 知識エントリーの `keywords` は自動選択モードの判定にも使われるため、汎用的すぎる語（「今日」「予定」など）を追加すると、雑談のつもりの発話でも結社マスターモードに切り替わるようになる。結社マスターモードに入ると `toolInstruction` の指示でモデルは関数の応答以外から答えなくなるため、「マンドリン」のような**結社と一般的な話題の両方で使われる語**を持たせると、一般的な質問に「分からない」と返すようになる
