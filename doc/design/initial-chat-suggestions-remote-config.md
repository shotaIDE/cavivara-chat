...
文言とアイコンは 1 枚のカードを構成する組であり、別々のパラメーターに分けると公開順によって文言とアイコンが食い違った状態が端末に届き得る。そのため、**1 つの JSON 型パラメーターにまとめ、常に一括で公開する**。パラメーター更新イベント（`onConfigUpdated`）も 1 回にまとまるため、アプリ側の扱いも単純になる。
...
### 方針 4: Remote Config で扱うのは候補の中身だけにする

Remote Config には**候補の全リスト**のみを持たせ、そこから実際に表示する分をランダムに選ぶ処理と、その件数（3 件）はアプリ側に残す。これは Remote Config 導入前の挙動（20 件から 3 件をランダム表示）をそのまま引き継ぐものである。

「どの 3 件を出すか」までを Remote Config で固定しないのは、毎回同じカードが並ぶとアプリを開き直す動機が薄れるためである。候補を多めに登録し、そこから毎回違う組み合わせを見せるほうが、文言の当たり外れも把握しやすい。

一方で**表示件数そのものは Remote Config の対象にしない**。件数はカードを横に並べるレイアウトに依存する値であり、変えたときに崩れないかは実機で確認しないと分からない。リリースなしで変えられることの利点より、確認なしに崩れた画面が配信されるリスクのほうが大きい。

また、**サジェストを定義したまま無効化するフラグ（`enabled`）も設けない**。出したくない文言は候補から消せば足り、「定義されているが表示されない」という状態を持たせるだけ、設定の読み手にとっても実装にとっても分岐が増える。
...
| `suggestions` | array\<Suggestion\> | ○ | 表示候補のリスト。この中から 3 件がランダムに表示される。空配列を指定すると、サジェスト自体を非表示にできる |
...
演奏会の告知のように期間限定で出したい文言は、時期が来たら候補に加え、終わったら消す。定義を残したまま無効化する仕組みは持たない。
...
- `InitialChatSuggestionsConfig`: `schemaVersion` / `suggestions`
...
抽選をウィジェットの `initState` ではなくプロバイダーに置くのは、コーディングルール（表示するデータのフィルタリングはプロバイダー内で行う）に合わせるためと、抽選結果を単体テストで検証できるようにするためである。表示件数の定数も、抽選を行うこのファイルに置く。
...
- ハードコードされた `_allSuggestions` を削除する（表示件数の定数はプレゼンターへ移す）
...
- **表示件数の変更**: レイアウトへの影響を実機で確認する必要があるため、アプリ側の定数として持つ
- **サジェストを定義したまま無効化すること**: 出したくない文言は候補から消す
...
- `InitialChatSuggestionsConfigService`
  ...
  - 未知のフィールドが無視される
  - `label` の欠落・空白のみ・オブジェクトでない要素・`label` の重複が除外され、他の要素は残る
...
- 表示されるのは候補のうち 3 件である。候補が 3 件だと毎回同じカードが並ぶため、抽選による新鮮さを保つには候補を十分多く登録する
