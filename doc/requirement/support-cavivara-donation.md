...
ユーザーとして、応援回数に応じた称号を獲得したい。なぜならば、自分の貢献度が称号として可視化されることで、参加意識や達成感が高まるからだ。

受入基準:

- 応援回数に応じて称号が付与される
- 設定画面に現在の称号が表示される
- 応援画面に現在の称号と次の称号までの進捗が表示される
- 称号データはローカル（SharedPreferences）に保存される
...
- 保存先: ローカルストレージ（SharedPreferences）
- 保存内容: 累計応援回数のみ
- 個別の応援履歴（日時・金額の詳細）は保存しない
- 称号は累計応援回数から動的に算出
